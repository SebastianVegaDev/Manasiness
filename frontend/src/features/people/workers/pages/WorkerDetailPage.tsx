...
function WorkerDetailPage() {
    ...
    useEffect(() => {
        async function fetchWorkerDetail(): Promise<void> {
            if (!id) {
                setHasError(true)
                setIsLoading(false)
                return
            ...
            try {
                ...
                const response: PersonHistoryApiResponse = await getWorkerById(id, {
                    sort: sortOrder,
                    page: currentPage,
                    offset: dayOffset,
                    period: "day"
    ...
    if (hasError || (!detail && !isLoading)) {
        return (
            <div>
                ...
                <button type="button" onClick={() => navigate("/dashboard/workers")}>
                    Back
                </button>
            </div>
        )
