...
export function mapCustomersToCards(customers: Person[] | null | undefined): PeopleCardItem[] {
    const safeCustomers = Array.isArray(customers) ? customers : []

    return safeCustomers.map((customer) => ({
        id: customer.id,
        name: customer.name,
        image: customer.image || FALLBACK_IMAGE,
        status: getPersonStatus(customer),
        details: [
...
export function mapCustomerToDetail(
    ...
    return {
        ...
        details: rows.map((row) => [
            ...
            getProductName(row),
            formatCurrency(Number(row.price ?? row.sale_price ?? row.salePrice ?? row.total ?? row.amount ?? 0), currencyCode),
...
export function mapCustomersTotalPage(response: PersonHistoryApiResponse): number {
