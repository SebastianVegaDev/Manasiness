...
export function mapOrdersToTables(
...
export function mapOrdersTotalPage(data: MovementApiWindowResponse | null): number {
    return Math.ceil(Number(data?.total_rows ?? data?.totalRows ?? 0) / 20)
}

export function mapOrdersWindowLabel(data: MovementApiWindowResponse | null): string {
    if (!data?.start_date && !data?.startDate) return ""
    return formatDateTime(data.start_date ?? data.startDate ?? null)
}

export function mapOrdersWindowState(data: MovementApiWindowResponse | null) {
    return {
...
export function mapSupplierOptions(data: BootstrapOption[] | null | undefined): MovementFieldOption[] {
    ...
    return safeData.map((item) => {
        ...
        return {
            ...
            label: isDefault ? `${item.name} - Cash order` : item.name,
            isDefault
        }
    })
}
