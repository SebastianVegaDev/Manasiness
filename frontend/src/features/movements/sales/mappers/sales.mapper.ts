...
export function mapSalesToTables(
...
export function mapSalesTotalPage(data: MovementApiWindowResponse | null): number {
    return Math.ceil(Number(data?.total_rows ?? data?.totalRows ?? 0) / 20)
}

export function mapSalesWindowLabel(data: MovementApiWindowResponse | null): string {
    if (!data?.start_date && !data?.startDate) return ""
    return formatDateTime(data.start_date ?? data.startDate ?? null)
}

export function mapSalesWindowState(data: MovementApiWindowResponse | null) {
    return {
        label: mapSalesWindowLabel(data),
        hasOlder: Boolean(data?.has_older ?? data?.hasOlder),
        hasNewer: Boolean(data?.has_newer ?? data?.hasNewer)
    }
}

export function mapCustomerOptions(data: BootstrapOption[] | null | undefined): MovementFieldOption[] {
    ...
    return safeData.map((item) => {
        ...
        return {
            ...
            label: isDefault ? `${item.name} - Cash sale` : item.name,
            isDefault
        }
    })
}
